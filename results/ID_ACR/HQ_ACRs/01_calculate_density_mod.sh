...
if [ $# -lt 5 ]; then
    echo "usage:sh *.sh <output_name> <bin_size> <step> <genome_size> <path_to_sample_file>"
    echo "
...
do
 ...
 total=$(cat $reads |wc -l)
 bedtools coverage -a ${name}_split_$n -b $reads -counts |awk '{print $1,$2,$3,$4,'$size'*$5/'$[total]'/($3-$2)}' OFS="\t" > ${name}_split_$n.$i.bgx
 for((i=1;i<$a;i++))
 do
 	j=$(($i-1))
  reads=sample_rep$i.Tn5
  total=$(cat $reads |wc -l)
  bedtools coverage -a ${name}_split_$n -b $reads -counts |awk '{print '$size'*$5/'$[total]'/($3-$2)}' OFS="\t" > ${name}_split_$n.$i.bg
