...
echo -n > ${name}/$chr.1.bg
 ...
 do
  ...
  awk '$1=="'$chr'"' ${reads} |awk '{print $1"-"$2}' OFS="\t" | uniq -c |sed "s/\ /\t/g" |awk '{print $NF,$(NF-1)}' OFS="\t" |awk '{print $1,'$size'*$2/'$[total]'}' OFS="\t" >> ${name}/$chr.1.bg
 done
...
echo "no contig"
 ...
 do
  reads=${sample[$x]}
  total=$(cat $reads |wc -l)
  grep -e "$ctg" ${reads} |awk '{print $1"-"$2}' OFS="\t" | uniq -c |sed "s/\ /\t/g" |awk '{print $NF,$(NF-1)}' OFS="\t" |awk '{print $1,'$size'*$2/'$[total]'}' OFS="\t" >>  $name/ctg.1.bg
 done
